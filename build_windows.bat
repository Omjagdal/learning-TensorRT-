...
:: 3. Bundle Offline Models
echo.
echo [3/5] Bundling Offline Models...
cd scripts
call bundle_ollama.bat
if %errorlevel% neq 0 (
    echo [ERROR] Bundling Ollama failed!
    exit /b %errorlevel%
)
call bundle_hf_models.bat
if %errorlevel% neq 0 (
    echo [ERROR] Bundling HF models failed!
    exit /b %errorlevel%
)
cd ..

:: 4. Copy Backend to Tauri Binaries
echo.
...
:: Since our PyInstaller output is a directory (because of large models), we copy the whole directory
...
:: 5. Build Tauri App
echo.
echo [5/5] Building Tauri App (.msi)...
cd frontend
