# 🤖 ISRA VISION OMI Assistant — Full Project Walkthrough
...
- The LLM is strictly instructed via `qwen_service.py` to:
  - Answer **only** from the provided context, without external knowledge.
  - Structure answers with specific font sizes (e.g., `<span style="font-size: 1.25em;">`) for key parameters.
  - Format parameter limits in a rigid markdown table (`Normal | Min Error | Min Warning | Max Warning | Max Error`).
  - Generate a **strict, linear Mermaid flowchart** (`A --> B --> C`) for almost all queries to map out steps visually.
  - Show step-by-step calculations explicitly.
